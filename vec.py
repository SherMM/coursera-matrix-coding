...
def equal(u,v):
    ...
    assert u.D == v.D
    return all([getitem(u,d) == getitem(v,d) for d in u.D])

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    return Vec({d for d in u.D},{d:getitem(u,d) + getitem(v,d) for d in u.D})
...
def scalar_mul(v, alpha):
    ...
    return Vec({d for d in v.D},{d:alpha*getitem(v,d) for d in v.D})

def neg(v):
    "Returns the negation of a vector"
    return Vec({d for d in v.D}, {d: -1*getitem(v,d) for d in v.D})
