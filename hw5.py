...
def morph(S, B):
    ...
    ieList = [] #list of 2 element vector tuples where tuple[0] == injected vector and tuple[1] == ejected vector
    copyS = S[:]
    for vector in B:
    	ieList.append((vector,exchange(copyS,B,vector)))
    return ieList
...
def find_matrix_inverse(A):
    ...
    I = Mat(A.D, {(d,d):one for d in A.D[0]})
